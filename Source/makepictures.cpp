...
template <typename a> std::__cxx11::string arrayn(a out[], int const& num_array)
{
...
int main(int argc,char *argv[])
{
    ...
    for(int i = 0; i <= 0; i++)//複数画像を描画する
    {
        ...
        /*****************************imageの設定***************************************************************************/
            ...
            bool ifeye;
            bool iflight;
            int p_eye_1_num = 2;  //サブパスで無限遠の点が生成されないにならない限り絶対にこの個数まで点が延長される
            int p_light_1_num =2;//サブパスで無限遠の点が生成されないにならない限り絶対にこの個数まで点が延長される
